...
```
For each batch of audio clips:

  ┌── DISCRIMINATOR STEP ────────────────────────────────────────┐
  │  1. Encode + decode → fake_audio (no grad on VAE)            │
  │  2. D(real_audio) → real_logits                              │
  │  3. D(fake_audio) → fake_logits                              │
  │  4. L_disc = LSGAN(real_logits, fake_logits)                 │
  │  5. backward() + accumulate                                   │
  └──────────────────────────────────────────────────────────────┘

  ┌── GENERATOR STEP ────────────────────────────────────────────┐
  │  1. z, μ, logvar, fake_audio = VAE(real_audio)               │
  │  2. distil_loss = WavLM(real_audio, z)                        │
  │  3. fake_out = D(fake_audio)  (grad flows through VAE)        │
  │  4. L_gen = λ_KL·KL + λ_adv·L_adv + λ_feat·L_feat           │
  │           + λ_distil·distil_loss                             │
  │  5. backward() + accumulate                                   │
  └──────────────────────────────────────────────────────────────┘

  Every grad_accum steps:
    clip_grad_norm → opt_g.step() → opt_d.step() → zero_grad()
```
...
## 10. Latent Normalisation Stats

After training the VAE, you must compute the **mean and standard deviation of the encoder
outputs** across the training set and save them. The CALM backbone requires zero-mean,
unit-variance latent inputs for stable training and inference.

From the paper (Table 14): `"center and normalize = ✓"` for both speech and music.

Without this step, the latent frames fed to the CALM backbone will have an arbitrary scale and
offset, making the backbone's job significantly harder and causing inconsistent generation quality.
...
### How normalisation is used in Stage 2 (CALM)

During CALM backbone training, every latent frame is normalised before being fed as input
and de-normalised after being predicted by the consistency head:

```python
# Load stats (computed once after VAE training)
stats = torch.load("checkpoints/speech_vae/latent_stats.pt")
latent_mean = stats["mean"].to(device)
latent_std  = stats["std"].to(device)

# In CALM training loop:
with torch.no_grad():
    z, mu, _ = vae.encode(audio, sample=False)          # raw latents
    z_norm   = normalise_latents(z, latent_mean, latent_std)  # normalised

# Feed z_norm as input sequence to the backbone Transformer
# Predict next z_norm frame with consistency head
# At inference: denormalise before passing to VAE decoder
z_pred     = consistency_head(z_norm_context)
audio_out  = vae.decode(denormalise_latents(z_pred, latent_mean, latent_std))
```

---

### Sanity check — what good stats look like

After a well-trained VAE with `λ_KL = 0.01`:

```
mean range:  [-0.3, +0.3]    (approximately zero-centred per dimension)
std  range:  [0.6,  1.8]     (close to unit variance, some spread is fine)
```

If you see `std > 5` for any dimension, the KL term is too weak and the posterior is
not close enough to `N(0, I)`. Increase `λ_KL` or train longer.
